...
#sudo apt-get update
#sudo apt-get install ca-certificates git -y
git checkout master
...
sleep 10
helm repo add dietfriends http://localhost:8080/repo
helm repo update

# inspect all by repo name
helm search dietfriends -l |  sed 1d | while read chart_name chart_version app_version desc; do
...
#  chart_name="`basename "$chart"`"
#  echo ">>> helm inspect dietfriends/$chart_name"
#  helm inspect dietfriends/"$chart_name"
#done
